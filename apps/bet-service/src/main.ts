...
async function bootstrap(): Promise<void> {
  ...
  const brokers = parseKafkaBrokers(process.env.KAFKA_BROKERS ?? process.env.KAFKA_BROKER);

  app.connectMicroservice<MicroserviceOptions>({
    transport: Transport.KAFKA,
    options: {
      client: {
        clientId: process.env.KAFKA_CLIENT_ID ?? 'bet-service',
        brokers,
      },
      consumer: {
        groupId: process.env.KAFKA_CONSUMER_GROUP_ID ?? 'bet-service-consumer',
  ...
  console.log(`Bet Service Kafka consumer started`);
}
